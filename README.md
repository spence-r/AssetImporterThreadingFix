...
Although the editor supports automatic LOD generation and texture baking, both the `BuildLOD` and `BakeLODTextures` methods create a _new_ `TaskDistributor` each time they are invoked. This creates `Environment.ProcessorCount * 2` threads on each invocation, which are never cleaned up after the completion of each LOD mesh/texture generation. 
